...
function AlignSign_(result: (FormattedLine | FormattedLine[])[], startIndex: number, endIndex: number, symbol: string, mode: string) {
    ...
    let betweenIfOpThenRegex: RegExp,
        closingConditionRegex: RegExp,
        closingConditionOpRegex: RegExp,
        openingConditionRegex: RegExp;
    if (symbol == "<=") {
    ...
    for (let i = startIndex; i <= endIndex; i++) {
        ...
        if (symbol == "<=") {
            ...
            if (line.regexCount(betweenIfOpThenRegex) > 0) skipCurrentLine = true;
            if (line.regexCount(closingConditionRegex) > 0) {
                insideCondition = false;
                if (line.regexCount(closingConditionOpRegex) > 0) skipCurrentLine = true;
            }
            if (line.regexCount(openingConditionRegex) > 0) insideCondition = true;
            if (insideCondition || skipCurrentLine) continue;
        }
